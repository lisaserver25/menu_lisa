...
# Versión: 6.5 (Título re-formateado para fácil edición)
# ==============================================================================
...
self_update() {
    ...
    if git clone --quiet --depth 1 "$REPO_URL" "$TEMP_DIR"; then
        local new_script_path="$TEMP_DIR/$SCRIPT_NAME"
...
show_menu() {
    while true; do
        ...
        # --- TÍTULO ASCII (Formato Multilínea para fácil edición) ---
        echo -e "        ${VERDE}██╗     ██╗███████╗ █████╗${FIN}"
        echo -e "        ${VERDE}██║     ██║██╔════╝██╔══██╗${FIN}"
        echo -e "        ${VERDE}██║     ██║███████╗███████║${FIN}"
        echo -e "        ${VERDE}██║     ██║╚════██║██╔══██║${FIN}"
        ...
        echo -e "  ${AMARILLO} 3)${FIN} ${GRIS}Índice de Herramientas (Pendiente)${FIN}"
        echo
        echo -e "  ${CYAN}Herramientas de Servidor${FIN}"
        echo -e "  ${AMARILLO} 4)${FIN} ${BLANCO_BRILLANTE}Analizador de URLs${FIN}"
        echo -e "  ${AMARILLO} 5)${FIN} ${GRIS}Instalación de Servidor Plex (Pendiente)${FIN}"
        echo -e "  ${AMARILLO} 6)${FIN} ${GRIS}Instalación de IPTV (Pendiente)${FIN}"
        echo -e "  ${AMARILLO} 7)${FIN} ${BLANCO_BRILLANTE}Convertir Logo a ASCII${FIN}"
        echo
        echo -e "  ${CYAN}Sistema e Información${FIN}"
        echo -e "  ${AMARILLO} 8)${FIN} ${VERDE}Actualizar Scripts${FIN}"
        echo -e "  ${AMARILLO} 9)${FIN} ${CYAN}Actualizar este Panel${FIN}"
        echo -e "  ${AMARILLO}50)${FIN} ${BLANCO_BRILLANTE}Reiniciar Servidor${FIN}"
        echo -e "  ${AMARILLO}99)${FIN} ${BLANCO_BRILLANTE}Aviso Legal${FIN}"
        echo -e "   ${AMARILLO}0)${FIN} ${BLANCO_BRILLANTE}Salir${FIN}"
        echo -e "${MAGENTA}---------------------------------------------------${FIN}"
